...
Since the DP, union-find and hashing categories landed, the app bundle is ~497 kB (~156 kB gzipped) and the build prints Vite's "chunks are larger than 500 kB" warning. Nothing is broken and the app is eagerly loaded on purpose (see `router/index.ts` on why views are not lazy chunks), but this is the point at which the next category should probably come with a code-splitting decision rather than without one.
