...
- The sandboxed iframe never sets `allow-same-origin` alongside `allow-scripts`, and never gains a visible rendering surface of its own — both are hard invariants, not tuning knobs.
- Every step received from the sandbox is validated (array of finite numbers; `comparing`/`swapping`/`sorted` are in-bounds indices; `done` is boolean) and messages are checked against `event.source === iframe.contentWindow` before anything reaches `BarChart` — malformed or spoofed input is dropped, not rendered.
